...
def crystal_struct_to_period_graph(crystal_fname, r_max, num_message_passing): 
    predict_crystal = ase.io.read(crystal_fname)
    crystal_config = config_from_atoms(predict_crystal)
    crystal_graph = graph_from_configuration(crystal_config,r_max)
    atoms = predict_crystal
    graph = jax.tree_util.tree_map(to_f32, crystal_graph)
    return atoms, graph

def crystal_atoms_to_period_graph(predict_crystal, r_max, num_message_passing): 
    crystal_config = config_from_atoms(predict_crystal)
    crystal_graph = graph_from_configuration(crystal_config,r_max)
    graph = jax.tree_util.tree_map(to_f32, crystal_graph)
    return graph
